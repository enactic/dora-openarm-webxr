...
class StereoPanel {
  ...
  constructor() {
    // Started early so the first frame is ready by session start.
    this.#ready = fetch("view_configuration")
      ...
      .catch((error) => {
        ...
        this.#configuration = FALLBACK_CONFIGURATION;
  ...
  render(session, space, frame) {
    ...
    const camera = this.#configuration.camera;
  ...
  // Resolves once the view configuration has been read.
  get ready() {
    return this.#ready;
  }
}

export function createStereoPanel() {
  return new StereoPanel();
}
